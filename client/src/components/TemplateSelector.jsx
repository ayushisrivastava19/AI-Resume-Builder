            ...
            <div className='absolute top-full w-xs p-3 mt-2 space-y-3 z-10 bg-white rounded-md border border-gray-200 shadow-sm'>
                {templates.map((template)=>(
                    <div key={template.id} onClick={()=> {onChange(template.id);
                        setIsOpen(false)}} className={`relative p-3 border rounded-md cursor-pointer transition-all ${selectedTemplate === template.id ? "border-gray-300 hover:border-gray-400 hover:bg-gray-100" : ""}`}>
                        ...
                        <div className="space-y-1">
                         ...
                         <div className='mt-2 p-2 bg-blue-50 rounded text-x-s text-gray-500 italic'>{template.preview}</div>
                        </div>
