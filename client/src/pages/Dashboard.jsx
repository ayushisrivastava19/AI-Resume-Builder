...
const Dashboard = () => {
 ...
 const uploadResume=async(event)=>{
  ...
  try {
   ...
   const resumeText = "Temporary resume content" 
      ...
      {showCreateResume && (
        <form onSubmit={createResume} onClick={()=> setShowCreateResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
          <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
            ...
            <input onChange={(e)=>setTitle(e.target.value)} value={title} type="text" placeholder='Enter resume title' className='w-full px-4 p-2 mb-4 focus:border-green-600 ring-green-600' required/>
         ...
         {showUploadResume && (
          ...
          <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
            ...
            <div>
              ...
              <input type="file" id="resume-input" hidden onChange={(e)=>setResume(e.target.files[0])} accept=".pdf,.doc,.docx"/>
            </div>
